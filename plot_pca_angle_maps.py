...
def correlation_stats(rows: list[dict], coordinates: dict, basis: str) -> dict:
    selected = [
        row for row in rows
        if row["fit_basis"] == basis and int(row["orientation_reliable"])
    ]
    ys = [coordinates[RESOURCE_ALIASES.get(row["village"], row["village"])][1] for row in selected]
    angles = [float(angle_from_horizontal(row)) for row in selected]
    pearson = pearsonr(ys, angles)
    spearman = spearmanr(ys, angles)
    return {
...
def angle_from_horizontal(row: dict) -> float:
    """Use traditional-display horizontal angles, with compatibility for earlier runs."""
    value = row.get("ellipse_angle_from_horizontal_signed_deg")
    if value not in (None, ""):
        return float(value)
    # Reflection into the traditional display maps the former raw-PC vertical
    # deviation exactly onto horizontal deviation.
    return float(row["ellipse_angle_from_vertical_signed_deg"])
...
def draw_map(ax, rows: list[dict], coordinates: dict, basis: str, angle_limit: float):
    ...
    scatter = ax.scatter(
        ...
        c=[angle_from_horizontal(row) for row in reliable],
        # Temperature-inspired convention: negative/southern-leaning angles
        # are warm red, positive angles are cool blue, and zero is neutral.
        cmap="coolwarm_r", vmin=-angle_limit, vmax=angle_limit,
    ...
    stats = correlation_stats(rows, coordinates, basis)
    ax.text(
        0.02, 0.98,
        f'Y vs horizontal angle (reliable only), n={stats["n_reliable"]}\n'
        f'Pearson r={stats["pearson_r"]:+.2f}, p={stats["pearson_p"]:.3g}\n'
        f'Spearman ρ={stats["spearman_rho"]:+.2f}, p={stats["spearman_p"]:.3g}',
        transform=ax.transAxes, va="top", fontsize=7,
        bbox={"boxstyle": "round,pad=0.25", "facecolor": "white", "edgecolor": "#999999", "alpha": 0.88},
        zorder=6,
    )
    return scatter, unstable, stats
...
def save_single(path: Path, rows: list[dict], coordinates: dict, basis: str, angle_limit: float) -> None:
    ...
    scatter, unstable, _ = draw_map(ax, rows, coordinates, basis, angle_limit)
    fig.colorbar(scatter, ax=ax, label="Signed angle from horizontal (degrees)", shrink=0.78)
    if unstable:
        ax.text(
...
def save_combined(path: Path, rows: list[dict], coordinates: dict, angle_limit: float) -> None:
    ...
    for ax, basis in zip(axes, ("vowels", "midpoints")):
        scatter, _, _ = draw_map(ax, rows, coordinates, basis, angle_limit)
    fig.colorbar(scatter, ax=axes, label="Signed angle from horizontal (degrees)", shrink=0.72)
...
def main() -> int:
    ...
    stats = [correlation_stats(rows, coordinates, basis) for basis in ("vowels", "midpoints")]
    with (args.output / "angle_y_correlations.csv").open("w", encoding="utf-8", newline="") as handle:
